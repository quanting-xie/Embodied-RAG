...
class JsonInterpreter:
    ...
    def get_image_path(self, node_id: str, dataset: str) -> Path:
        """Construct the correct image path for a given node"""
        image_filename = f"streetview_{int(node_id):04d}.jpg"
        return self.dataset_base_dir / dataset / "images/merged" / image_filename

    def read_json_file(self, json_path: str) -> Dict[str, Any]:
        """Read and parse the JSON file containing street view data"""
        ...
        with open(json_path, 'r') as file:
            try:
                ...
                print(f"Found {len(nodes)} street view points")
                return nodes
    ...
    async def prepare_batch_requests(self, nodes: list, dataset: str, batch_size: int = 100) -> list:
        ...
        for i in range(0, total_nodes, batch_size):
            ...
            for node in batch_nodes:
                ...
                image_path = self.get_image_path(str(node['id']), dataset)
                
                if not image_path.exists():
                    print(f"Skipping node {node.get('id', 'unknown')}: Image not found at {image_path}")
                    continue
                    ...
                    request = {
                        ...
                        "body": {
                            ...
                            "messages": [
                                {
                                    "role": "system",
                                    "content": "You are an environment analyst. Analyze this street view panorama and describe the environment."
                                },
                                {
                                    "role": "user",
                                    "content": [
                                        {
                                            "type": "text",
                                            "text": """Analyze this street view panorama and provide:
                                            1. A short abstract name (4-6 words) summarize the scene
                                            2. A detailed description of the environment, including:
                                               - List objects/elements in the scene
                                               - Read out the signs and labels in the scene

                                            Format your response as:
                                            NAME: <4-6 word abstract name>, use only nouns.
                                            DESCRIPTION: <detailed urban environment description>"""
                                        },
                                        {
                                            "type": "image_url",
                                            "image_url": {
            ...
            if batch_status.status == 'completed':
                ...
                for node in chunk_nodes:
                    ...
                    try:
                        ...
                        # Get the correct image path from the merged images
                        image_path = node.get('images', {}).get('merged', {}).get('path', '')
                        ...
                        node_data[node_id] = {
                            ...
                            "position": {
                                'x': longitude,  # Use longitude for x
                                'y': latitude,   # Use latitude for y
                                'z': 0.0
                            },
                            "timestamp": node.get('date', ''),
                            "level": 0,
                            "pano_id": node.get('pano_id', ''),
                            "base_heading": node.get('base_heading', 0)
                        }
                    except (KeyError, TypeError, ValueError) as e:
                        print(f"Error extracting coordinates for node {node_id}: {str(e)}")
                        print(f"Node data: {node}")
                        # Use default coordinates if extraction fails
                        node_data[node_id] = {
                            "type": "street_view",
                            "image_path": image_path,
                            "name": caption_data["name"],
...
async def main():
    ...
    try:
        # Process the street view JSON file
        dataset = "CMU_500"  # or "tokyo"
        json_path = f"/Users/danielxie/Embodied-RAG_datasets/{dataset}/metadata/streetview_graph.json"
        output_path = f"{dataset}_streetview_graph.gml"
        
        # Verify image directory exists
        image_dir = interpreter.dataset_base_dir / dataset / "images/merged"
        if not image_dir.exists():
            raise FileNotFoundError(f"Image directory not found: {image_dir}")
            
        # Verify first image exists
        test_image = image_dir / "streetview_0000.jpg"
